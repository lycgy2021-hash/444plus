...
// manifest (name + short_name = GitLab).
func manifestIsGitLab(body []byte) bool {
	s := string(body)
	return strings.Contains(s, `"name": "GitLab"`) || strings.Contains(s, `"name":"GitLab"`) ||
		strings.Contains(s, `"short_name": "GitLab"`) || strings.Contains(s, `"short_name":"GitLab"`)
}
