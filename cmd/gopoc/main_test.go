...
func TestCLIEndToEndAndOutputIsolation(t *testing.T) {
	...
	if len(result.Targets) != 1 || len(result.Findings) != 30 || result.Findings[0].Verdict != model.VerdictNotFound || result.Findings[1].Verdict != model.VerdictDetected {
...
func TestCLIActiveConfigurationAndCancellationReport(t *testing.T) {
	...
	if err := json.Unmarshal(stdout.Bytes(), &result); err != nil || !result.Cancelled || len(result.Findings) != 30 || result.Findings[0].Reason != "cancelled" {
