...
//
// Nine boundaries are enforced by the TYPE SHAPES below (and, for the most
// important one, by a PACKAGE boundary), not merely by comment, because a
// boundary a bad actor (or an over-eager future implementer) can route around
// by adding one field — or one file to the same package — is not a boundary:
//
...
//  4. Raw state evidence and state identity are two different hashes, never
...
//     Candidate/Evidence's Provenance privacy elsewhere in this package): all
//     fields are unexported, NewStateFingerprint copies its Facts input, and
//     every getter returns a copy. This closes a real tamper path: a mutable
//     `Facts map[string]string` field could be edited after construction
//     without StateFingerprintHash changing to match, desynchronizing the
//     "hash of the facts" from the facts themselves.
//  6. Recovery is registry-backed (boundary 3, via actionauth.BoundRecovery)
...
//  7. StateTransition carries FACTS ONLY — no Unexpected/Vulnerable/Severity/
...
//  8. ExplorationBudget has NO "0 = unlimited" escape hatch. Every bound must
//     be strictly positive or the budget is INVALID.
//  9. S10 invents no second "who may define correct behavior" system —
//     whatever future producer judges a StateTransition worth a hypothesis
//     reuses S9's ExpectationSource unchanged. Also recorded here (no
//     corresponding type; it constrains execution behavior, not data shape):
//     within one ExplorationScope, v1 exploration is SINGLE SESSION and
//     SERIAL — at most one in-flight action at a time.
...
// (boundary 4) and is itself immutable once built (boundary 5): every field is
// unexported, so nothing can mutate Facts after construction and thereby
// desynchronize it from StateFingerprintHash, which the caller must have
// already computed over these SAME facts before calling NewStateFingerprint
// (hashing is a producer-specific canonicalization decision, exactly as S9's
// caseArtifactHash/comparisonHash are producer-computed, not computed by a
// shared constructor).
...
// NewStateFingerprint constructs an immutable StateFingerprint. facts is
// copied so the caller's map cannot be mutated afterward to desynchronize it
// from stateFingerprintHash.
func NewStateFingerprint(scopeHash, rawStateArtifactHash, stateFingerprintHash string, facts map[string]string) StateFingerprint {
	...
	return StateFingerprint{
		scopeHash:            scopeHash,
		rawStateArtifactHash: rawStateArtifactHash,
		stateFingerprintHash: stateFingerprintHash,
		facts:                copied,
	}
}
